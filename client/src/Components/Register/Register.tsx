...
function Register(): JSX.Element {
  ...
  const [users, setUsers] = useState<UserModel[]>(
    store.getState().UserState.usersST
  );
...
Invalid email
  ...
  const usersMap = async () => {
    ...
    users.map((user) => {
      console.log(user.ID);
      ID1 == user.ID ? (response = true) : console.log(false);
    });
    return response;
  };

  const change = async () => {
    if(await usersMap() === true) {
      setAlert(true);
    }else{
      if(!Email.includes("@")){
        setAlert1(true);
  ...
  const check = () => {
    if (step2) {
      return (
        <>
          <Tab label="step 1" value="1" disabled />
      ...
      return (
        <>
          ...
          <Tab label="step 2" value="2" disabled />
        </>
      );
    }
  };
        ...
        <div className="form">
          <TabContext value={value}>
            ...
            <TabPanel value="1">
              ...
              <div className="Alert1">{alertOn1()}</div>
